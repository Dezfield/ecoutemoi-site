...
function PreferencesForm({ initial }: { initial: NotificationPreferences }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setDraft(saved);
      setMessage({ tone: 'success', text: 'Настройки сохранены и применяются сервером до отправки push.' });
    } catch (error) {
    ...
    <form onSubmit={(event) => void submit(event)} aria-busy={saving}>
      <section className="panel" aria-labelledby="categories-title">
        ...
        <Toggle
          ...
          description="Редкие обновления Écoute Moi. По умолчанию отключены; никаких сообщений о дефиците или давлении."
