...
/** The sanction (and its id) always comes from get_my_safety_center_v1; nothing is typed by the user. */
  ...
  return (
    <form className="form appeal-form" onSubmit={validate} noValidate>
      ...
      <div className="field">
        ...
        />
        <p id={hintId} className="field-hint">Опишите контекст спокойно и по существу, от 20 до 1500 символов.</p>
      </div>
      ...
      >
        <p>Обращение попадёт в очередь модерации. Для одного ограничения можно отправить одно обращение.</p>
      </ConfirmDialog>
    </form>
  );
}
          ...
          <>
            <p className="panel-text">
              <span className={`status-pill ${data.activeSanction ? 'status-warn' : 'status-ok'}`}>
                {data.activeSanction ? sanctionLabels[data.activeSanction.kind] : 'Активных ограничений нет'}
              </span>
            </p>
            {data.activeSanction ? (
              <>
            ...
            {data.hasMoreReports ? <p className="panel-note">Показаны последние 20 жалоб.</p> : null}

            {data.recentAppeals.length ? (
              <>
                <h3 className="panel-subtitle">Мои обращения</h3>
                <ul className="list">
                  {data.recentAppeals.map((appeal) => (
                    <li key={appeal.id} className="list-row">
                      <span className="list-main">
                        <span className="list-title">{appeal.sanctionKind ? sanctionLabels[appeal.sanctionKind] : 'Ограничение'}</span>
                        <span className="list-meta">{formatDateTime(appeal.createdAt)}</span>
                      </span>
                      <span className="status-pill">{appealLabels[appeal.status]}</span>
                    </li>
                  ))}
                </ul>
              </>
...
function ActiveSessions() {
  ...
  return (
    <section className="panel" aria-labelledby="sessions-title">
      ...
      <p className="panel-text">Устройства и браузеры, на которых сохранён вход в ваш аккаунт, включая приложение.</p>
  ...
  return (
    <>
      <PageHeader eyebrow="Безопасность" title="Безопасность">
        <p>Статус аккаунта, жалобы, обращения и устройства, на которых выполнен вход.</p>
      </PageHeader>
      <SafetyStatus />
      <ActiveSessions />
