...
The web app in `app/` is a separate bundle with its own boundary: only the Supabase URL and publishable key in the client, all data access under existing RLS/RPC, PKCE auth with a single explicit callback, internal-only post-login redirects, noindex. See `docs/WEB_APP_DEPLOYMENT.md`. Legal placeholders and missing contact are content release blockers, not hidden completed features.
