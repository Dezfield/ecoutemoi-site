...
**Backend source of truth:** `Dezfield/ecoutemoi-mobile`, branch `main`, commit `fbe9058ec82ecf75b24878393efb29045f6753e0` (2026-09-26, merge of PR #10 "communication deletion guards"; includes PR #5 "account deletion lifecycle + stale-JWT guards" and PR #8). Line numbers below refer to that commit; PR #10 only added `supabase/migrations/20260925120000_communication_deleted_account_guards.sql`, its test and a CI step, so every earlier file and line is unchanged since `9cb6500`.
...
`submit_user_report`, `block_user` and a few mobile-only chat RPCs are not wrapped. `submit_user_report` still accepts a report from a prepared account under its existing membership and rate-limit checks; that is outside this change. The web does not rely on the frontend for this: after a successful deletion it clears the session and shows `/account-deleted`.
