...
import { defaultAppUrl, pages, title, description } from '../src/content';
...
// Личный кабинет link target. Production always uses the approved app domain; previews may override it.
const appUrlInput = production ? defaultAppUrl : (process.env.APP_URL || defaultAppUrl);
const appUrlParsed = new URL(appUrlInput);
if (!['https:', 'http:'].includes(appUrlParsed.protocol) || appUrlParsed.pathname !== '/' || appUrlParsed.search || appUrlParsed.hash || appUrlParsed.username || appUrlParsed.password) throw new Error('APP_URL must be a plain HTTP(S) origin');
if (production && appUrlParsed.protocol !== 'https:') throw new Error('Production APP_URL must use HTTPS');
const appUrl = `${appUrlParsed.origin}/`;
...
for (const route of ['/', ...Object.keys(pages), '/404']) {
  ...
  const html = template.replace('<html lang="ru">', `<html lang="ru" data-base="${base}" data-route="${route}" data-app-url="${escape(appUrl)}">`).replace(/<title>.*?<\/title>/, `<title>${escape(pageTitle)}</title>`).replace('</head>', `${meta}</head>`).replace('<div id="root"></div>', `<div id="root">${renderToString(<App path={route} base={base} appUrl={appUrl}/>)}</div>`);
